...
echo -e "${BLUE}${BOLD}"
echo "========================================================"
echo "🦅 BUZZARD POWER SUITE - UNIVERSAL LINUX INSTALLER v0.5.0"
echo "========================================================"
echo -e "${RESET}"
...
if command -v apt-get &> /dev/null; then
    ...
    sudo apt-get install -y -qq tlp powertop x11-xserver-utils libnotify-bin python3-gi gir1.2-appindicator3-0.1 power-profiles-daemon linux-tools-common acpi python3-pip python3-yaml || true

elif command -v pacman &> /dev/null; then
    echo -e "${BLUE}▶ Using PACMAN Package Manager (Arch/Manjaro/EndeavourOS)...${RESET}"
    sudo pacman -Sy --needed --noconfirm tlp powertop xorg-xrandr libnotify python-gobject libappindicator-gtk3 power-profiles-daemon acpi python-pip python-yaml || true

elif command -v dnf &> /dev/null; then
    echo -e "${BLUE}▶ Using DNF Package Manager (Fedora/RHEL/Alma)...${RESET}"
    sudo dnf install -y tlp powertop xrandr libnotify python3-gobject libappindicator-gtk3 power-profiles-daemon kernel-tools acpi python3-pip python3-pyyaml || true

elif command -v zypper &> /dev/null; then
    echo -e "${BLUE}▶ Using ZYPPER Package Manager (openSUSE)...${RESET}"
    sudo zypper install -y tlp powertop xrandr libnotify-tools python3-gobject power-profiles-daemon acpi python3-pip python3-PyYAML || true

else
    echo -e "${YELLOW}Notice: Unknown package manager. Please ensure python3, tlp, powertop, and power-profiles-daemon are installed manually.${RESET}"
fi
...
SUDOERS_CONTENT="ALL ALL=(ALL) NOPASSWD: /usr/bin/tee /sys/*, /usr/bin/tlp *, /usr/bin/powertop *, /usr/bin/powerprofilesctl *"
